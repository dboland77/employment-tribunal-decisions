...
def build_fts_index():
    ...
    """)
    conn.commit()
    n = conn.execute("SELECT COUNT(*) FROM decisions_fts").fetchone()[0]
    if n == 0:
...
.result h2 a { color: #1a3a5c; text-decoration: none; }
...
.decision-meta h2 { font-size: 1.15rem; margin-bottom: 1rem; color: #1a3a5c; line-height: 1.4; }
...
<div class="container">
  ...
  {% if results %}
    {% for r in results %}
    <div class="result">
      <h2><a href="/decision{{ r['slug'] }}">{{ r['title'] or r['slug'] }}</a></h2>
      <div class="meta">
        {% if r['decision_date'] %}<span>{{ r['decision_date'] }}</span>{% endif %}
        {% if r['tribunal_type'] %}
          <span class="tag {{ r['tribunal_type'] }}">
            {{ 'Employment Tribunal' if r['tribunal_type']=='et' else 'Appeal Tribunal' }}
          </span>
        {% endif %}
...
<div class="container">
  ...
  <div class="decision-meta">
    ...
    <table>
      <tr><td>Tribunal</td>
          <td>{{ 'Employment Appeal Tribunal' if d['tribunal_type']=='eat' else 'Employment Tribunal' }}</td></tr>
...
def search():
    ...
    return render_template_string(
        SEARCH_TMPL,
        style=STYLE, results=rows, q=q, tribunal=tribunal, country=country,
...
def decision(slug):
    ...
    if row is None:
        abort(404)
    return render_template_string(
        DETAIL_TMPL, style=STYLE, d=dict(row), json=json,
    )
